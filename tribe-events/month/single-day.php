...
<?php
    $daylink = esc_url( tribe_get_day_link( $day['date'] ) );
    $daylink = substr($daylink,0,-1);
    $daylink = substr($daylink, 0,-10)."?tribe_event_display=list&tribe-bar-date=".substr($daylink, -10);
    //print_r($daylink);
?>
	<?php if ( $day['total_events'] > 0 && tribe_events_is_view_enabled( 'day' ) ) : ?>
		<a href="<?php echo $daylink/*esc_url( tribe_get_day_link( $day['date'] ) )*/; ?>"><?php echo $day['daynum'] ?></a>
	<?php else : ?>
		<?php echo $day['daynum'] ?>
	<?php endif; ?>
